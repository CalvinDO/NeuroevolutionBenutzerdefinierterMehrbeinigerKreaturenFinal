...
namespace Unity.MLAgentsExamples {
    ...
    public class JointDriveController : MonoBehaviour {
        ...
        /// </summary>
        public int SetupBodyPart(Transform t) {

            int amountOutputs = 0;


            ConfigurableJoint currentJoint = t.GetComponent<ConfigurableJoint>();

            CWRotationInterfaceRotationalFreedom rotationalFreedom = CWRotationInterfaceRotationalFreedom.Nothing;

            if (currentJoint != null) {

                if (currentJoint.angularYMotion == ConfigurableJointMotion.Locked) {

                    if (currentJoint.angularXMotion == ConfigurableJointMotion.Locked) {

                        rotationalFreedom = CWRotationInterfaceRotationalFreedom.Nothing;
                    }
                    else {
                        rotationalFreedom = CWRotationInterfaceRotationalFreedom.X;
                    ...
                    if (currentJoint.angularXMotion == ConfigurableJointMotion.Locked) {
                        rotationalFreedom = CWRotationInterfaceRotationalFreedom.Y;

                        amountOutputs++;
                    }
                    else {
                        rotationalFreedom = CWRotationInterfaceRotationalFreedom.XY;
            ...
            var bp = new BodyPart {
                ...
                joint = currentJoint,
                rotationalFreedom = rotationalFreedom,
            ...
            if (!bp.groundContact) {
                ...
                bp.groundContact.crawlerController = gameObject.GetComponent<CWCreatureController>();
            }
            else {
                bp.groundContact.crawlerController = gameObject.GetComponent<CWCreatureController>();
            }
