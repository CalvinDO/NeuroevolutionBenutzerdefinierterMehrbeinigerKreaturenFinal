...
namespace RuntimeGizmos {
    ...
    public class TransformGizmo : MonoBehaviour {
        ...
        //Calvins modification=
        public LayerMask selectionMask = Physics.DefaultRaycastLayers;
        //public LayerMask selectionMask = LayerMask.GetMask("RotationHandle");
        ...
        AxisInfo axisInfo;
        //CWEdit: cheesy dirtyfix
        private bool setAxisZNextFrame;
        ...
        Transform tempTargetForTagChange;

        void Awake() {
            myCamera = GetComponent<Camera>();
            SetMaterial();

            //manually set transformType & TranslatingType to only space type rotate
            this.transformType = TransformType.Rotate;
            translatingType = transformType;

            TransformGizmo.instance = this;
        }
        ...
        void Update() {
            //CWEdit: No UndoRedo or setspacetyp updates.
            //there is only rotate and that stays
            //which is defined in awake
			...
			*/
            ...
            if (manuallyHandleGizmo) {
                ...
                //CWEdit: Avoid blocking of direct click-moving handles due to overlap of other axis'
                //SetNearAxis();
            }

            if (this.setAxisZNextFrame) {
                this.SetAxisZ();
                this.setAxisZNextFrame = false;
            }
            else {
                GetTarget();
            }
        ...
        void OnPostRender() {
            //CWEdit:
            //don't render any gizmos for the moment
            return;
            ...
            //CWEdit: only draw z for dummy one circle handle
            //change only comment next 3 lines
            //redone because they are still there
        ...
        void TransformSelected() {
            if (mainTargetRoot != null) {
                //CWEdit: 
                // not != Axis.None but == Axis.Z
                if (nearAxis == Axis.Z && (Input.GetMouseButtonDown(0) || this.setAxisZNextFrame)) {
                    //Debug.Log(this.translatingType);
        ...
        IEnumerator TransformSelected(TransformType transType) {
                ...
                if (previousMousePosition != Vector3.zero && mousePosition != Vector3.zero) {
                    if (transType == TransformType.Move) {
                        ...
                        for (int i = 0; i < targetRootsOrdered.Count; i++) {
                            ...
                            //CWEdit: insert next line, tag change to transformed for Y value copy
                            target.tag = "HandleIsTransformed";
                            //CWEdit: insNLine temptarget
                            this.tempTargetForTagChange = target;
            ...
            //CWEdit: insert next line, tag change to default for stop Y value copy
            tempTargetForTagChange.tag = "Untagged";
        ...
        void GetTarget() {
            if (nearAxis == Axis.None && Input.GetMouseButtonDown(0)) {
                //CWEdit: no add select, only 1 is allowed to by selected
                //old lines:
                ...
                RaycastHit hitInfo;
                if (Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, /* Calvins edit= selectionMask */
            LayerMask.GetMask("RotationHandle"))) {
        ...
        public void AddTarget(Transform target, bool addCommand = true) {
            if (target != null) {
                ...
                //CWEdit: no highlights. comment next line
                //AddTargetHighlightedRenderers(target);

                SetPivotPoint();
                //CWEdit: moved down next line
                //SetPivotPoint();
                //CWEdit: cheesy dirtyfix for next frame axis set
                this.setAxisZNextFrame = true;


                //CWEdit
                //inserted next line for direct manipulation

                //CWEdit: some lines to refresh transform
        ...
        void HandleNearest(TransformType type, float xClosestDistance, float yClosestDistance, float zClosestDistance, float allClosestDistance, float minSelectedDistanceCheck) {
            if (type == TransformType.Scale && allClosestDistance <= minSelectedDistanceCheck) SetTranslatingAxis(type, Axis.Any);
                ...
                //CWEdit: hide next line so no inball rotate
